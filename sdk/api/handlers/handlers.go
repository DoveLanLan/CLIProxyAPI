...
func (h *BaseAPIHandler) WriteErrorResponse(c *gin.Context, msg *interfaces.ErrorMessage) {
	...
	applyManagedErrorHeaders(c.Writer.Header(), msg)
...
func applyManagedErrorHeaders(dst http.Header, msg *interfaces.ErrorMessage) {
	if dst == nil || msg == nil || msg.Error == nil {
		return
	}
	type managedHeaderProvider interface {
		ManagedHeaders() http.Header
	}
	var provider managedHeaderProvider
	if !errors.As(msg.Error, &provider) || provider == nil {
		return
	}
	// Keep the bypass contract narrow: Retry-After is generated by CPA for
	// local backpressure. Arbitrary upstream headers still require passthrough.
	if value := strings.TrimSpace(provider.ManagedHeaders().Get("Retry-After")); value != "" {
