...
func TestAppendWebsocketEventTruncatesAtLimit(t *testing.T) {
	var builder strings.Builder
	payload := bytes.Repeat([]byte("a"), wsBodyLogMaxSize*2)

	appendWebsocketEvent(&builder, "request", payload)

	got := builder.String()
	if len(got) > wsBodyLogMaxSize {
		t.Fatalf("body length = %d, want <= %d", len(got), wsBodyLogMaxSize)
	}
...
func TestAppendWebsocketEventNoGrowthAfterLimit(t *testing.T) {
	var builder strings.Builder
	appendWebsocketEvent(&builder, "request", bytes.Repeat([]byte("a"), wsBodyLogMaxSize*2))
	before := builder.Len()

	appendWebsocketEvent(&builder, "response", []byte(`{"type":"response.created"}`))

	if builder.Len() != before {
		t.Fatalf("body grew after cap: before=%d after=%d", before, builder.Len())
	}
}
