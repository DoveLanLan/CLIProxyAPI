...
- Changing `docker-compose.yml` unless runtime execution proves it is strictly necessary.
...
5. The user receives explicit instructions for configuring official Codex Team accounts via the repo’s Codex login flow rather than via `config.yaml`. (Verify: final response)
...
- Config/flags: `config.yaml` is mounted by `docker-compose.yml`; official Codex Team setup later uses `--codex-device-login` or `--codex-login` against the same config/auth mount.
