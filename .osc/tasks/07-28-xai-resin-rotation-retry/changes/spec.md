...
The exact-402 retry budget and the single pre-response network retry budget are
independent. A network failure may consume its one replay before or after a
lease rotation, but the network replay must keep the currently routed auth and
Account. A later exact 402 may still consume a configured rotation retry, and a
post-rotation network failure may still consume the one network replay if it
...
- Production releases must use the immutable GHCR image selected by GitHub
  Actions. An explicit `CLI_PROXY_IMAGE` supplied by the workflow takes
  precedence over the server `.env`, while `.env` remains the fallback for
  manual invocation without an explicit image.
- A standalone shell regression must cover both image-precedence branches
  without invoking Docker or SSH.
