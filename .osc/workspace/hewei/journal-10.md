...
What changed: Added `USAGE_QUERY_LIMIT: ${CPA_MANAGER_USAGE_QUERY_LIMIT:-100}` to `deploy/compose.production.yml` and documented the override in `deploy/README.md`.
...
Next steps: Deploy the compose change to the VPS and restart `cpa-manager`. If the page still times out at `100`, inspect CPA-Manager SQLite `raw_json` row sizes.
