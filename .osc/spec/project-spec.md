...
- **Open Questions (max 1):** The exposed OpenCode credential is disabled and absent from active production config, but provider-side revocation/regeneration still requires an authenticated OpenCode control-plane session.
...
9. Optional xAI Resin routing derives a stable anonymous Account as `xai-<HMAC-SHA256(identity-key, auth-id)[:16]>` at runtime. Explicit auth proxies remain higher priority; Resin and EgressProxyPool are mutually exclusive; secret-file failures fail closed. A pre-response Resin network failure may replay once with the same auth and derived Account so Resin can use its newly rotated lease; a second or mid-response failure remains request-scoped without cooling xAI credentials or falling back to another auth, EgressProxyPool, or the global proxy. — Evidence: `internal/runtime/executor/helps/xai_resin_proxy.go`, `internal/runtime/executor/xai_proxy_pool_executor.go`, `deploy/XAI_RESIN_PROXY_SETUP_CN.md` (Documented; confidence: High; updated 2026-07-28)
